---
title: "Jobs -> The recovery regaining its pace"
...
read_csv("D:\\Documents\\R directory\\labor market\\nfp_revisions.csv") %>% 
  ...
  mutate(date_released = case_when(estimate == "1st" ~ month,
         ...
         rank_index = dense_rank(index) - 14, ## need to edit this number
         txt_labels = paste0(values, "k")) %>%
  filter(month > "2021-02-28") %>%
...
dat_chg %>% 
  ...
  mutate(avg_curr_yr = case_when(key == "nfp_chg_sa" & date > "2021-06-30" ~ avgs_curr_yr$avg_sa, # edit date
                                 key == "nfp_chg_nsa" & date > "2021-06-30" ~ avgs_curr_yr$avg_nsa)) %>% # edit date
  filter(date > "2021-02-28") -> dat_chg_avg ## edit date
...
dat_long <- dat_ph %>% 
  ...
  mutate(idx = ifelse(recess == "1948", counter, #update date here
               ...
               ifelse(recess == "2020" & date == "2021-10-01", counter + 170, NA)))))))))))))))))))))))))))))))) %>% 
...
raw_dat %>% 
  ...
  mutate(since_feb20 = case_when(date == ymd("2020-02-01") ~ PAYEMS,
         ...
         since_now =   case_when(date == ymd("2021-10-01") ~ PAYEMS, #edit this
                                 date != ymd("2021-10-01") ~ NA_real_)) -> dat #edit this
...
row_start_now <- which(grepl(ymd("2021-10-01"), dat_fut$date)) #edit this
...
dat_fut %>% 
  mutate(since_feb20 = case_when(date < ymd("2020-02-01") ~ PAYEMS,
         ...
         since_now = case_when(date < ymd("2021-10-01") ~ PAYEMS, #edit this
                              date >= ymd("2021-10-01") ~ pull(dat[row_start_now,"since_now"]) + #edit this
...
df_long %>% 
  filter(date > "2017-12-01" & date < "2024-04-01") %>% 
  ...
  scale_x_date(expand = c(0,0),
               limits = c(ymd("2019-01-01"), ymd("2024-04-01")),
               date_labels = "'%y",
               date_breaks = "1 year") +
  scale_y_continuous(labels = fmt_dcmls(0)) +
  labs(title = ctitle_1, subtitle = unit_label, caption = source_1) +
...
BL_chart <- BL %>% 
  ...
  geom_bar(aes(fill = key, y = value, x = date),
           ...
           stat = "identity",
           width = 32) +
  facet_zoom(xy = date > "2018/8/1" & date < "2021/11/01", #edit this
             zoom.size = .4) +
