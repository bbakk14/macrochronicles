---
title: "Prices -> Diverging headline and core"
date: 2022-07-13T21:13:14-05:00
draft: false
editor_options:
...
dat %>% 
  ...
  geom_point(data = tail(dat,3),
             aes(date, values),
             size = 3) +
  geom_text_repel(data = tail(dat,3),
...
dat_wide[,-1] %>%
  ...
  mutate(mean = rowMeans(., na.rm = TRUE),
         median = rowMedians(as.matrix(.), na.rm = TRUE),
...
dats_final[[2]] %>% 
  filter(!key %in% c("mean", "median", "sd", "mean_plussd", "mean_lesssd", "x_inter")) %>% 
  ...
  geom_vline(data = dats_final[[2]] %>% filter(key == "mean_lesssd"), 
             ...
             linetype = "dotted",
             size = 1) +
  geom_vline(data = dats_final[[2]] %>% filter(key == "median"), 
             aes(xintercept = values, frame = date, color = "Median"), 
             linetype = "dashed",
             size = 1) +
  geom_text(data = dats_final[[2]] %>% filter(key == "Bread"),
            ...
            family = "Open Sans") +
  scale_color_manual(values = c(cpal("red"), cpal("border grey"), cpal("border grey"), cpal("purple"))) +
  ...
  guides(color = guide_legend(nrow = 2, 
                              override.aes = list(linetype = c("dashed", "dotted", "dotted", "dashed"))),
...
anim1 <- animate(chart1, renderer = gifski_renderer(loop = T), nframe = 290,
                 width = 615, height = 470, type = "cairo", end_pause = 80)
