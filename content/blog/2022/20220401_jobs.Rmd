---
title: "Jobs -> Prime-age participation on fire"
...
Job gains in March beat expectations. Gains in January and February were both revised up. There are now roughly 1.6 million fewer jobs now than before the pandemic as the recovery in the labor market continues steadily.
...
read_csv("D:\\Documents\\R directory\\labor market\\nfp_revisions.csv") %>% 
  ...
  mutate(date_released = case_when(estimate == "1st" ~ month,
         ...
         rank_index = dense_rank(index) - 19, ## need to edit this number
         txt_labels = paste0(values, "k")) %>%
  filter(month > "2021-07-31") %>%
...
df %>% 
  mutate(date_released = case_when(Estimate == "Re-benchmarked" & month == "2021-12-01" ~ as.Date("2022-01-01"), # need to edit this
                                   TRUE ~ date_released),
         released = case_when(Estimate == "Re-benchmarked" & month == "2021-12-01" ~ "Jan '22", #need to edit this
                                   TRUE ~ released)) -> df
...
dat_chg %>% 
  ...
  mutate(avg_curr_yr = case_when(key == "nfp_chg_sa" & date > "2021-11-30" ~ avgs_curr_yr$avg_sa, # edit date
                                 key == "nfp_chg_nsa" & date > "2021-11-30" ~ avgs_curr_yr$avg_nsa)) %>% # edit date
  filter(date > "2021-07-31") -> dat_chg_avg ## edit date
...
The unemployment rate declined to 3.6%. The strength in the prime-age employment to population ratio is notable, while the rate of workers who are part time for economic reasons was flat. The recovery in labor force participation picking up is good to see.
...
Transportation and warehousing as well as professional service jobs continue to stand out as industries where employment is growing at a strong pace, even though they're already well above pre-pandemic levels of employment. Retail jobs have started to surpass pre-pandemic levels as well. 
...
The chart below shows the strong pace of this recovery relative to other recessions. It's striking how much of straight line the recovery has maintained over the last year. The labor market is 1.6 million jobs away from its pre-pandemic peak.
...
dat_long <- dat_ph %>% 
  ...
  mutate(idx = ifelse(recess == "1948", counter, #update date here
               ...
               ifelse(recess == "2020" & date == "2021-12-01", counter + 174, 
               ifelse(recess == "2020" & date == "2022-01-01", counter + 176,
               ifelse(recess == "2020" & date == "2022-02-01", counter + 178, 
               ifelse(recess == "2020" & date == "2022-03-01", counter + 180, NA))))))))))))))))))))))))))))))))))))) %>%
...
raw_dat %>% 
  ...
  mutate(since_feb20 = case_when(date == ymd("2020-02-01") ~ PAYEMS,
         ...
         since_now =   case_when(date == ymd("2022-03-01") ~ PAYEMS, #edit this
                                 date != ymd("2022-03-01") ~ NA_real_)) -> dat #edit this
...
row_start_now <- which(grepl(ymd("2022-03-01"), dat_fut$date)) #edit this
...
dat_fut %>% 
  mutate(since_feb20 = case_when(date < ymd("2020-02-01") ~ PAYEMS,
         ...
         since_now = case_when(date < ymd("2022-03-01") ~ PAYEMS, #edit this
                              date >= ymd("2022-03-01") ~ pull(dat[row_start_now,"since_now"]) + #edit this
...
df_long %>% 
  filter(date > "2017-12-01" & date < "2024-06-01") %>% 
  ...
  scale_x_date(expand = c(0,0),
               limits = c(ymd("2019-01-01"), ymd("2024-06-01")),
               date_labels = "'%y",
               date_breaks = "1 year") +
  scale_y_continuous(labels = fmt_dcmls(0)) +
  labs(title = ctitle_1, subtitle = unit_label, caption = source_1) +
...
BL_chart <- BL %>% 
  ...
  geom_bar(aes(fill = key, y = value, x = date),
           ...
           stat = "identity",
           width = 32) +
  facet_zoom(xy = date > "2018/8/1" & date < "2022/04/01", #edit this
             zoom.size = .4) +
