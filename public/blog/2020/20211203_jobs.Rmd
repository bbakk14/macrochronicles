---
title: "Jobs -> Payrolls miss expectations but broader measures improve"
date: 2021-12-03T21:13:14-05:00
draft: false
---
&nbsp;
&nbsp;
...
Job gains in November came in below expectations. A more holistic view of the data, however, shows that the recovery in the labor market remains on track. The mixed signals appear in large part to stem from varying strengths of conditions in household survey and establishment survey data.
...
read_csv("D:\\Documents\\R directory\\labor market\\nfp_revisions.csv") %>% 
  ...
  mutate(date_released = case_when(estimate == "1st" ~ month,
         ...
         rank_index = dense_rank(index) - 15, ## need to edit this number
         txt_labels = paste0(values, "k")) %>%
  filter(month > "2021-03-31") %>%
...
dat_chg %>% 
  ...
  mutate(avg_curr_yr = case_when(key == "nfp_chg_sa" & date > "2021-07-30" ~ avgs_curr_yr$avg_sa, # edit date
                                 key == "nfp_chg_nsa" & date > "2021-07-30" ~ avgs_curr_yr$avg_nsa)) %>% # edit date
  filter(date > "2021-03-31") -> dat_chg_avg ## edit date
...
Despite the lower than expected net number of job gains, the unemployment rate, labor force participation rates, and employment-to-population ratios all improved in a way that made it look like a month of strong labor market conditions.
...
Transportation and warehousing as well as professional service jobs continue to stand out as industries where employment is growing at a strong pace, even though they're already well above pre-pandemic levels of employment. Leisure and hospitality job gains have slowed, likely reflecting the ongoing effects of the pandemic. 
...
dat_long <- dat_ph %>% 
  ...
  mutate(idx = ifelse(recess == "1948", counter, #update date here
               ...
               ifelse(recess == "2020" & date == "2021-11-01", counter + 172, NA))))))))))))))))))))))))))))))))) %>% 
...
raw_dat %>% 
  ...
  mutate(since_feb20 = case_when(date == ymd("2020-02-01") ~ PAYEMS,
         ...
         since_now =   case_when(date == ymd("2021-11-01") ~ PAYEMS, #edit this
...
BL_chart <- BL %>% 
  ...
  geom_bar(aes(fill = key, y = value, x = date),
           ...
           stat = "identity",
           width = 32) +
  facet_zoom(xy = date > "2018/8/1" & date < "2021/12/01", #edit this
             zoom.size = .4) +
