---
title: "Jobs -> Solid adds but weakening participation"
...
Job gains in June beat expectations, with gains of 372k jobs. Gains in April and May were revised down by 68k and 6k, respectively.
...
read_csv("D:\\Documents\\R directory\\labor market\\nfp_revisions.csv") %>% 
  ...
  mutate(date_released = case_when(estimate == "1st" ~ month,
         ...
         rank_index = dense_rank(index) - 22, 
...
df %>% 
  mutate(date_released = case_when(Estimate == "Re-benchmarked" & month == "2021-12-01" ~ as.Date("2022-01-01"), # need to edit this
                                   TRUE ~ date_released),
         released = case_when(Estimate == "Re-benchmarked" & month == "2021-12-01" ~ "Jan '22", 
                                   TRUE ~ released)) -> df
...
The unemployment rate remained steady at 3.6%. Labor force participation and employment-to-population ratios weakened in June relative to May. 
...
The chart below shows the strong pace of this recovery relative to other recessions. It's striking how much of straight line the recovery has maintained over the last year. The labor market is half million jobs away from its pre-pandemic peak.
...
dat_long <- dat_ph %>% 
  ...
  mutate(idx = ifelse(recess == "1948", counter, #update date here
               ...
               ifelse(recess == "2020" & date == "2022-03-01", counter + 180,
               ifelse(recess == "2020" & date == "2022-04-01", counter + 182,
               ifelse(recess == "2020" & date == "2022-05-01", counter + 184,
               ifelse(recess == "2020" & date == "2022-06-01", counter + 186, NA)))))))))))))))))))))))))))))))))))))))) %>%
...
source_1 <- "Source: BLS, FRED, Betterment."
